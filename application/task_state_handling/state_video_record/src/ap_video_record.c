...
//prototype
	...
	void ap_video_record_md_tick(INT32U *md_tick,INT32U state);
	INT8S ap_video_record_md_active(INT32U *md_tick,INT32U state);
...
INT8S ap_video_record_func_key_active(INT32U event)
{
	...
	if (video_record_sts & VIDEO_RECORD_BUSY) {
		...
		if((event == MSG_APQ_VIDEO_RECORD_ACTIVE) || (event == MSG_APQ_MD_SET)||(event == MSG_APQ_POWER_KEY_ACTIVE))
		{
...
#else
        ap_peripheral_auto_off_force_disable_set(0); // dominant add,function key event, must disable timer OSD 
#endif
        video_calculate_left_recording_time_enable();
...
void ap_video_record_cycle_reply_action(void)
{
	...
	if (next_file_info.file_handle != -1) {
		...
		if( (total_size <= 1024) && (ap_state_config_record_time_get() == 4) )|| (ap_state_config_record_time_get() >= 5){   //lx 2015-08-25
			size = 300;
		}else{
...
void ap_video_record_reply_action(STOR_SERV_FILEINFO *file_info_ptr)
{
	...
	if (value == 0) {
        ...
        #else
        bkground_del_disable(1); //0 // back ground auto delete enable   //lx
        #endif
