...
static INT8U g_osd_display = 1; //默认显示OSD
...
void ap_storage_service_osd_flag_set(INT8U flag)
{
	g_osd_display = flag;
}

INT8U ap_storage_service_osd_flag_get(void)
{
	return g_osd_display;
}
...
{
	INT8U temp[11] = {0, 1, 2, 3, 4, 5, 6, 7, 10, 15, 20}; //7,10,15,   Liuxi modified in 2015-01-17, 
    INT32U	SDC_MB_Size;

	if(ap_state_config_video_resolution_get() >= 2) { //720P and below
		BkDelThreadMB = temp[ap_state_config_record_time_get()]*50;	//50MB/Min //100MB per minute
		...
		BkDelThreadMB = temp[ap_state_config_record_time_get()]*50;	//50MB/Min //200MB per minute
		SDC_MB_Size = drvl2_sd_sector_number_get()/2048;
...
static INT32S save_example_time_to_disk(void)
{
	...
	p = (INT8U*)addr;
	sprintf((char *)p, (const char *)"2016-05-01 23:59:59   ");
	#if OSD_SWITCH_MODE
...
extern void OpenPrintMsgSet(INT8U);
static INT32S rtc_time_get_and_start(void)
{
	...
	data -= 0x30;
	wtemp += data*1;
	if((wtemp > 2027) || (wtemp < 2017)) goto Fail_Return;
