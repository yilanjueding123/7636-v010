...
INT32S video_recording_start(vid_rec_args_t* pVidRecArgs)
{
	...
	{
		//lastFIFOIdx = 3;
		lastFIFOIdx = ap_state_resource_OSD_video_position_y_get();
	}
	else
	{
		lastFIFOIdx = 2;
...
extern INT16U state_usb_read_uvc_flag(void);
extern INT16U ap_peripheral_uvc_flag(void);
extern INT8U FIFO_LINE_LN ;
extern INT8U ap_storage_service_osd_flag_get(void);
void cpu_draw_time_osd(TIME_T current_time, INT32U target_buffer, INT8U draw_type, INT8U state)
{
	...
	#if OSD_SWITCH_MODE
	if (state_usb_read_uvc_flag() != 0x02)
	{//UVC 需要显示水印
		if (ap_storage_service_osd_flag_get() != 1) return; 
	}
	#endif
	...
	if(((state & 0xF) == (STATE_VIDEO_RECORD & 0xF)) || ((state & 0xF) == (STATE_VIDEO_PREVIEW & 0xF))) {
		gap_1 = 0;	//3;
		gap_2 = 20;
		str_info.font_color = ap_state_resource_OSD_font_color_get();	//white		//0x5050;	//green
		...
		if((state & 0xF) == (STATE_VIDEO_RECORD & 0xF))
		{
			...
			else
			{
				...
				{	// The 2nd part of date stamp 
					str_info.pos_y = 0; 
					str_info.font_offset_h = 1;//20
				}
				else
				{	// The first part of date stamp
					str_info.pos_y = 31;//12
					str_info.font_offset_h = 0;
				}
			}
		} 
		...
		{	// Capture Photo
			...
			{
				//str_info.pos_y = 651;
				if ((ap_state_config_pic_size_get()!=5)&&(ap_state_config_pic_size_get()!=8)&&(ap_state_config_pic_size_get()!=9))
				{
					str_info.pos_x = ap_state_resource_time_stamp_position_x_get()-320;
				}
		...
		gap_1 = 2;
		gap_2 = 8;
		str_info.font_color = ap_state_resource_OSD_font_color_get();	//white
...
/*	
	//logo print
	{
		str_info.font_color = 0x2FE7;	//green		
		
		ap_state_resource_char_draw_osd('J', (INT16U *) target_buffer, &str_info, draw_type, (state == (STATE_AUDIO_RECORD & 0xF))?2:1);
		str_info.pos_x += gap_1;
		ap_state_resource_char_draw_osd('o', (INT16U *) target_buffer, &str_info, draw_type, (state == (STATE_AUDIO_RECORD & 0xF))?2:1);
		str_info.pos_x += gap_1;
		ap_state_resource_char_draw_osd('y', (INT16U *) target_buffer, &str_info, draw_type, (state == (STATE_AUDIO_RECORD & 0xF))?2:1);
		str_info.pos_x += gap_1;
		ap_state_resource_char_draw_osd('H', (INT16U *) target_buffer, &str_info, draw_type, (state == (STATE_AUDIO_RECORD & 0xF))?2:1);
		str_info.pos_x += gap_1;
		ap_state_resource_char_draw_osd('o', (INT16U *) target_buffer, &str_info, draw_type, (state == (STATE_AUDIO_RECORD & 0xF))?2:1);
		str_info.pos_x += gap_1;
		ap_state_resource_char_draw_osd('n', (INT16U *) target_buffer, &str_info, draw_type, (state == (STATE_AUDIO_RECORD & 0xF))?2:1);
		str_info.pos_x += gap_1;
		ap_state_resource_char_draw_osd('e', (INT16U *) target_buffer, &str_info, draw_type, (state == (STATE_AUDIO_RECORD & 0xF))?2:1);
		str_info.pos_x += gap_1;
		ap_state_resource_char_draw_osd('s', (INT16U *) target_buffer, &str_info, draw_type, (state == (STATE_AUDIO_RECORD & 0xF))?2:1);
		str_info.pos_x += gap_1;
		ap_state_resource_char_draw_osd('t', (INT16U *) target_buffer, &str_info, draw_type, (state == (STATE_AUDIO_RECORD & 0xF))?2:1);
	}
	
	str_info.pos_x = ap_state_resource_time_stamp_position_x_get();//时间标签的x坐标,默认是从0开始
	str_info.font_color = ap_state_resource_OSD_font_color_get();	//white		//0x5050;	//green
*/
	if ((state_usb_read_uvc_flag() != 0x02)||(UVC_OSD_SHOW_TIME == 1))
	{
	...
	//UVC OSD
	//------------------------------------------------------------------------------------------------------------------------------------------
	if (state_usb_read_uvc_flag() == 0x02) // UVC
	{
		if (ap_peripheral_uvc_flag() != 1) return;
		str_info.pos_x = 10;//时间标签的x坐标,默认是从0开始    uvc 
		ap_state_resource_char_draw_osd((PRODUCT_NUM%10000)/1000+0x30, (INT16U *) target_buffer, &str_info, draw_type, (state == (STATE_AUDIO_RECORD & 0xF))?2:1);
		str_info.pos_x += gap_1;
		ap_state_resource_char_draw_osd((PRODUCT_NUM%1000)/100+0x30, (INT16U *) target_buffer, &str_info, draw_type, (state == (STATE_AUDIO_RECORD & 0xF))?2:1);
		str_info.pos_x += gap_1;
		ap_state_resource_char_draw_osd((PRODUCT_NUM%100)/10+0x30, (INT16U *) target_buffer, &str_info, draw_type, (state == (STATE_AUDIO_RECORD & 0xF))?2:1);
		str_info.pos_x += gap_1;
		ap_state_resource_char_draw_osd((PRODUCT_NUM%10)/1+0x30, (INT16U *) target_buffer, &str_info, draw_type, (state == (STATE_AUDIO_RECORD & 0xF))?2:1);
		str_info.pos_x += gap_2;
		
		ap_state_resource_char_draw_osd('v', (INT16U *) target_buffer, &str_info, draw_type, (state == (STATE_AUDIO_RECORD & 0xF))?2:1);
		str_info.pos_x += gap_1;
		ap_state_resource_char_draw_osd((PROGRAM_VERSION_NUM%1000)/100+0x30, (INT16U *) target_buffer, &str_info, draw_type, (state == (STATE_AUDIO_RECORD & 0xF))?2:1);
		str_info.pos_x += gap_1;
		ap_state_resource_char_draw_osd('.', (INT16U *) target_buffer, &str_info, draw_type, (state == (STATE_AUDIO_RECORD & 0xF))?2:1);
		str_info.pos_x += gap_1;
		ap_state_resource_char_draw_osd((PROGRAM_VERSION_NUM%100)/10+0x30, (INT16U *) target_buffer, &str_info, draw_type, (state == (STATE_AUDIO_RECORD & 0xF))?2:1);
		str_info.pos_x += gap_1;
		ap_state_resource_char_draw_osd((PROGRAM_VERSION_NUM%10)/1+0x30, (INT16U *) target_buffer, &str_info, draw_type, (state == (STATE_AUDIO_RECORD & 0xF))?2:1);
	}
	//------------------------------------------------------------------------------------------------------------------------------------------
